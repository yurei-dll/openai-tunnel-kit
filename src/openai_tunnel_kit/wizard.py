...
pre{white-space:pre-wrap;background:#0d0d0d;border:1px solid #333;border-radius:8px;padding:14px;min-height:50px}.warn{color:#ffbd66}.secret{font-family:monospace}.req{color:#ff5c5c;font-weight:800}.req[hidden]{display:none}.hint{color:#999;font-size:13px}
...
<form id="setup"><fieldset><legend>1. Admin access and auto-populate</legend><div class="grid">
<div><label>Admin API key <span class="req" data-req="admin-key" title="Required for automatic credentials and tunnel mutations">*</span></label><input class="secret" name="admin_api_key" type="password" autocomplete="off"><span class="hint">A value entered here overrides the saved global credential.</span></div>
<div><label>System wallet</label><p><label><input type="checkbox" name="use_saved_admin_key" checked> Use saved admin key</label></p><p><label><input type="checkbox" name="save_admin_key"> Save entered key globally in system wallet</label></p></div>
<div><label>Project <span class="req" data-req="project" title="Required for automatic credentials">*</span></label><select name="project_id"><option value="">Auto-populate projects with admin key</option></select></div>
<div><label>&nbsp;</label><button type="button" id="auto-populate">Auto-populate</button> <button type="button" id="forget-admin-key">Forget saved key</button></div>
<div style="grid-column:1/-1" class="hint">Uses the entered or saved Admin API key to load active projects, and restores any previously verified organization setting. Organization and workspace IDs are never guessed.</div>
</div></fieldset>
<fieldset><legend>2. MCP configuration</legend><div class="grid">
<div style="grid-column:1/-1"><label>mcp.json (recommended) <span class="req" data-req="mcp-file" title="Required unless a manual target is entered">*</span></label><input name="mcp_file" type="file" accept="application/json,.json"><span class="hint">Required unless you enter the MCP command or Server URL below. A file containing one mcpServers entry also supplies the profile name.</span></div>
<div><label>Profile name <span class="req" title="Required">*</span></label><input name="profile" required></div>
...
<div><label>MCP command <span class="req" data-req="mcp-command" title="Required for local command mode without mcp.json">*</span></label><input name="command" placeholder="node /absolute/path/dist/index.js"></div>
<div style="grid-column:1/-1"><label>MCP server URL <span class="req" data-req="mcp-url" title="Required for Server URL mode without mcp.json">*</span></label><input name="mcp_url" placeholder="https://mcp.example.com/mcp"></div></div></fieldset>
<fieldset><legend>3. Runtime credentials</legend><div class="grid">
...
<div><label>Existing runtime key (manual mode only) <span class="req" data-req="runtime-key" title="Required in existing credential mode">*</span></label><input class="secret" name="runtime_api_key" type="password" autocomplete="off"></div></div></fieldset>
<fieldset><legend>4. Tunnel control plane</legend><div class="grid">
...
<div><label>Existing tunnel ID <span class="req" data-req="tunnel-id" title="Required when reusing a tunnel">*</span></label><input name="tunnel_id" placeholder="tunnel_..."></div>
<div><label>Organization IDs (comma-separated) <span class="req" data-req="organization" title="An organization or workspace ID is required">*</span></label><input name="organization_ids" placeholder="org_..."></div>
<div><label>Workspace IDs (comma-separated; never guess) <span class="req" data-req="workspace" title="An organization or workspace ID is required">*</span></label><input name="workspace_ids" placeholder="ws_..."></div>
<div style="grid-column:1/-1" class="hint"><span class="req" data-req="scope">*</span> At least one organization or workspace ID is required when creating or attaching a tunnel.</div>
<div><label>Tunnel name</label><input name="tunnel_name" placeholder="Defaults to the profile name"><span class="hint">Leave blank to match the profile name automatically.</span></div>
<div><label>Description</label><input name="tunnel_description" value="Managed by openai-tunnel-kit"></div></div></fieldset>
<fieldset><legend>5. Installation</legend>
...
const setRequiredMarker=(name,visible)=>{for(const marker of document.querySelectorAll(`[data-req="${name}"]`))marker.hidden=!visible};
const updateRequiredMarkers=()=>{const hasFile=form.elements.mcp_file.files.length>0,mode=form.elements.target_mode.value,hasManualTarget=(mode==='command'?form.elements.command.value:form.elements.mcp_url.value).trim().length>0;setRequiredMarker('mcp-file',!hasFile&&!hasManualTarget);setRequiredMarker('mcp-command',!hasFile&&mode==='command');setRequiredMarker('mcp-url',!hasFile&&mode==='url');const credentialMode=form.elements.credential_mode.value,tunnelMode=form.elements.tunnel_mode.value;setRequiredMarker('admin-key',credentialMode==='automatic'||['attach','provision'].includes(tunnelMode));setRequiredMarker('project',credentialMode==='automatic');setRequiredMarker('runtime-key',credentialMode==='existing');setRequiredMarker('tunnel-id',tunnelMode==='existing');const needsScope=['attach','provision'].includes(tunnelMode),hasScope=form.elements.organization_ids.value.trim().length>0||form.elements.workspace_ids.value.trim().length>0;for(const name of ['organization','workspace','scope'])setRequiredMarker(name,needsScope&&!hasScope)};
for(const eventName of ['input','change'])form.addEventListener(eventName,updateRequiredMarkers);updateRequiredMarkers();
form.elements.mcp_file.addEventListener('change',async()=>{const file=form.elements.mcp_file.files[0];if(!file||form.elements.profile.value.trim())return;try{const content=JSON.parse(await file.text()),servers=content&&content.mcpServers;if(servers&&typeof servers==='object'&&!Array.isArray(servers)){const names=Object.keys(servers);if(names.length===1){form.elements.profile.value=names[0];out.textContent=`Profile name populated from mcp.json: ${names[0]}`}}}catch(_err){/* Setup validation reports malformed JSON on submission. */}});
...
fetch('/api/preferences',{headers:{'x-wizard-token':token}}).then(async r=>{const body=await r.json();if(r.ok&&body.last_organization_id&&!form.elements.organization_ids.value){form.elements.organization_ids.value=body.last_organization_id;updateRequiredMarkers()}});
document.querySelector('#auto-populate').addEventListener('click',async()=>{out.textContent='Auto-populating available settings…';const [projectResponse,preferenceResponse]=await Promise.all([fetch('/api/projects',{method:'POST',headers:{'content-type':'application/json','x-wizard-token':token},body:JSON.stringify(adminCredentialData())}),fetch('/api/preferences',{headers:{'x-wizard-token':token}})]);const body=await projectResponse.json(),preferences=await preferenceResponse.json();if(!projectResponse.ok){out.textContent=JSON.stringify(body,null,2);return}const select=form.elements.project_id;select.innerHTML='';for(const p of body.projects){const o=document.createElement('option');o.value=p.id;o.textContent=`${p.name} (${p.id})`;select.appendChild(o)}let restoredOrganization=false;if(preferenceResponse.ok&&preferences.last_organization_id&&!form.elements.organization_ids.value){form.elements.organization_ids.value=preferences.last_organization_id;restoredOrganization=true}if(body.admin_key_saved){form.elements.admin_api_key.value='';form.elements.save_admin_key.checked=false;form.elements.use_saved_admin_key.checked=true}updateRequiredMarkers();out.textContent=`Auto-populated ${body.projects.length} active project(s).${restoredOrganization?' Restored the last verified organization ID.':''}${body.admin_key_saved?' Admin key saved in the system wallet.':''}`});
